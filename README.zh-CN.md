...
| [繁体 · 香港](rewild-hk/SKILL.md) | 65 | 书面语／粤文语域分支、「您」、港式文言虚词、中英夹杂 |
| [繁体 · 台湾](rewild-tw/SKILL.md) | 65 | 日语底层结构受保护、台湾套语、公文骨架 |
...
两份模式库都不是简体版的翻译。45 条简体模式是一条一条过的：香港保留 29、改写 15、删 1；台湾保留 25、改写 19、删 1。然后各自再加 16 条简体模式库里根本没有的地区模式。其中有几条「改写」其实是**撤销**——因为原规则会把当地正常的文字判成 AI 味：
...
两次独立的基准测试都撞上了这件事，两次都只能靠人把成品并排读一遍才发现。

脚本本身有一百三十多条回归测试，覆盖它历史上出过的每一个 bug：
